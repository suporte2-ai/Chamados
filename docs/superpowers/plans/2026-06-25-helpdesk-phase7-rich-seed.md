# Fase 7 — Seed Rico: Plano de Implementação

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reescrever `backend/prisma/seed.js` para popular o banco com dados ricos de demonstração — 5 setores, 20 usuários, 200 tickets com timeline, ~80 comentários, 20 ideias e 30 notificações.

**Architecture:** Reescrita completa de um único arquivo. As funções existentes são expandidas in-place; duas novas funções (`seedTicketComments`, `seedNotifications`) são adicionadas. Nenhuma migration, endpoint ou teste novo.

**Tech Stack:** Node.js, Prisma 5, bcrypt, PostgreSQL.

## Global Constraints

- Senha de todos os usuários: `Senha123!` (bcrypt, rounds=10)
- Role names exatos: `'Administrador'`, `'Gestor'`, `'Técnico/Atendente'`, `'Usuário Final'`
- `seedSectors()` retorna objeto nomeado `{ ti, rh, financeiro, operacoes, juridico }` — NÃO array posicional
- `seedUsers(roles, sectors)` recebe objeto nomeado, não array
- `seedTickets(users, categories, sectors)` faz re-fetch final com `prisma.ticket.findMany` e retorna o array
- Tickets: 200, últimos 90 dias (`90 * 24 * 60` minutos atrás)
- Pool de status: `['ABERTO','EM_ANDAMENTO','AGUARDANDO','RESOLVIDO','RESOLVIDO','RESOLVIDO','FECHADO','FECHADO','FECHADO','FECHADO']`
- IdeaComments: **não criar** (omitidos intencionalmente)
- `seedIdeas` retorna array de ideias criadas
- Notificações: exatamente 30 (9 TICKET_ASSIGNED + 8 TICKET_STATUS_CHANGED + 7 TICKET_COMMENT + 6 IDEA_STATUS_CHANGED), metade `isRead: true`

---

### Task 1: Reescrever seed.js completo

**Files:**
- Modify: `backend/prisma/seed.js` (reescrita total)

**Interfaces:**
- Produces:
  - `seedSectors()` → `{ ti, rh, financeiro, operacoes, juridico }`
  - `seedUsers(roles, sectors)` → `User[]` (20 usuários)
  - `seedTickets(users, categories, sectors)` → `{ id, title, status, requesterId, assignedToId }[]` (200 tickets)
  - `seedIdeas(users)` → `{ id, title, status }[]` (20 ideias)
  - `main()` → void, imprime sumário no console

- [ ] **Step 1: Substituir todo o conteúdo de `backend/prisma/seed.js`**
...
- [ ] **Step 2: Rodar o seed e verificar saída**

```bash
cd backend && npm run seed
```

Saída esperada (valores exatos de tickets/comentários podem variar por aleatoriedade):
```
Iniciando seed rico...
  Banco limpo.
  ...
  20 usuários criados.
  200 tickets criados.
  Comentários de tickets criados (~40% dos tickets).
  20 ideias criadas.
  30 notificações criadas.

Seed concluído com sucesso!
  admin@helpdesk.com    / Senha123!  (Administrador)
  gestor1@helpdesk.com  / Senha123!  (Gestor - TI)
  gestor2@helpdesk.com  / Senha123!  (Gestor - RH)
  tecnico1@helpdesk.com / Senha123!  (Técnico - TI)
  usuario1@helpdesk.com / Senha123!  (Usuário Final - TI)
```

Se o comando falhar com erro de `Invalid prisma.X.create()`, verifique se o campo em questão existe no schema (`backend/prisma/schema.prisma`).

- [ ] **Step 3: Verificar contagens via suite de testes existente**

Os testes de integração existentes limpam o banco com `beforeAll` próprio e não dependem do seed. Rode a suite para garantir que nenhum teste quebrou:

```bash
cd backend && npx jest --no-coverage --runInBand
```

Saída esperada: a mesma baseline de antes do seed — 164/165 testes passam (1 falha pré-existente em `ticket-core.test.js:54`, não relacionada a esta tarefa).

- [ ] **Step 4: Commit**

```bash
git add backend/prisma/seed.js
git commit -m "feat: replace seed with rich demo data (200 tickets, 20 users, 30 notifications)"
```
