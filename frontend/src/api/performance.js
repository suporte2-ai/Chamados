...
export const performanceApi = {
  summary: (from, to, sectorId) => api.get('/api/performance/summary', { params: { from, to, ...(sectorId ? { sectorId } : {}) } }).then(r => r.data),
  drilldown: (id, params) => api.get(`/api/performance/users/${id}/drilldown`, { params }).then(r => r.data),
  volume: (from, to, sectorId) => api.get('/api/performance/volume', { params: { from, to, ...(sectorId ? { sectorId } : {}) } }).then(r => r.data),
